...
// Player content component that uses search params
function PlayerContent() {
  const router = useRouter();
  
  // Instead of using search params, we'll use a simpler approach with static content
  // This avoids the server rendering issues with useSearchParams
  const title = 'Summer Vibes';
  const artist = 'DJ Sunshine';
  const coverUrl = 'https://images.unsplash.com/photo-1534131707746-25d604851a1f?q=80&w=1000';
...
export default function IpodPlayer() {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[var(--background)] overflow-hidden">
      ...
      {/* Wrap the content that uses search params in a Suspense boundary */}
      <Suspense fallback={<div className="flex justify-center items-center h-[calc(100vh-64px)]">Loading player...</div>}>
        <PlayerContent />
      </Suspense>
    </div>
  );
